...
l = 0.08;               % m   - distance from axel to center of pendulum mass
m = 0.33;               % kg  - robot upper load (pendulum)
M = 1.180;              % kg  - mass of lower chassis
...
B = [0; 1/M; 0; 1/(l*M)];  % input matrix
B = B ./ R;           % adjust input from force to wheel torque.
...
%% Gp
G_wheelspeed = Gx / R * s; % -> (Ymech'): rad/s /torque
Gp_x_char = n*Km*Ye*G_wheelspeed; % characteristic eq.
Gp_x = Gp_x_char/ (1+ Gp_x_char) * R/s;
Gp_t = n*Ye*Km*Gt;
T.Gp = Gp_t;
X.Gp = Gp_x;
...
KT.Ki = T.Ki;
KT.Kd = T.Kd;
T.K=[1, T.Kp, T.Ki, T.Kd];

% Ktune(KT, T.G, T.H, T.p, 0.05, 5)

KX.Kp = X.Kp;
KX.Ki = X.Ki;
KX.Kd = X.Kd;
X.K=[1, X.Kp, X.Ki, X.Kd];

% 

% 6.8250, 13.0417, 4.6583
T.Kmt = [2.6917, 5, 1.35];
X.Kmt = [4.197*14.8,13.575*9.8,1.0250];

T.D3 = T.Kp*T.Kmt(1) + T.Ki/s*T.Kmt(2) + T.Kd * -T.p*s/(s-T.p) * T.Kmt(3);
X.D3 = X.Kp*X.Kmt(1) + X.Ki/s*X.Kmt(2) + X.Kd * -X.p*s/(s-X.p)* X.Kmt(3);

Ttest.Kp = T.Kp*T.Kmt(1);
Ttest.Ki = T.Ki*T.Kmt(2);
Ttest.Kd = T.Kd*T.Kmt(3);

Xtest.Kp = X.Kp*X.Kmt(1);
Xtest.Ki = X.Ki*X.Kmt(2);
Xtest.Kd = X.Kd*X.Kmt(3);

Ktune(Ttest, T.G, T.H, T.p, 0.05, 5)
% Ktune(Xtest, X.G, X.H, X.p, 0.05, 5)
