...
function Ktune(K, G, H, p, Ts, OSu)
    ...
    %
    % KP SLIDER CONFIG
    sliderKp = uislider(fig, 'Position', [100, 260, 600, 4], 'Limits', [0, 10], 'Value', 1);
    labelKp = uilabel(fig, 'Position', [100, 280, 600, 20], 'Text', 'Kp Multiplier: 1');
    
    % KI SLIDER CONFIG
    sliderKi = uislider(fig, 'Position', [100, 160, 600, 4], 'Limits', [0, 10], 'Value', 1);
    labelKi = uilabel(fig, 'Position', [100, 180, 600, 20], 'Text', 'Ki Multiplier: 1');
    
    % KD SLIDER CONFIG
    sliderKd = uislider(fig, 'Position', [100, 60, 600, 4], 'Limits', [0, 20], 'Value', 1);
    labelKd = uilabel(fig, 'Position', [100, 80, 600, 20], 'Text', 'Kd Multiplier: 1');
    ...
    function updatePlot()
        ...
        labelKp.Text = sprintf('Kp Multiplier: %.4f,  Result: %.4f', KpMultiplier, K.Kp*KpMultiplier);
        labelKi.Text = sprintf('Ki Multiplier: %.4f,  Result: %.4f', KiMultiplier, K.Ki*KiMultiplier);
        labelKd.Text = sprintf('Kd Multiplier: %.4f,  Result: %.4f', KdMultiplier, K.Kd*KdMultiplier);
